...
let cube; // Declare cube variable
let cubeOutline; // Declare cubeOutline variable
...
function init() {
    ...
    cube = new THREE.Mesh(cubeGeometry, cubeMaterial); // Create cube mesh
    scene.add(cube); // Add cube to the scene

    // Create an outline for the cube
    const cubeOutlineMaterial = new THREE.LineBasicMaterial({ color: 0xffa500 });
    const cubeOutlineGeometry = new THREE.EdgesGeometry(cubeGeometry); // Create edges geometry from the cube geometry
    cubeOutline = new THREE.LineSegments(cubeOutlineGeometry, cubeOutlineMaterial); // Create line segments for the outline
    ...
    // Add keyboard event listener for cube movement
    document.addEventListener('keydown', (event) => {
        switch (event.key) {
            case 'ArrowUp': // Move up along the y-axis
                cube.position.y += 1;
                break;
            case 'ArrowDown': // Move down along the y-axis
                cube.position.y -= 1;
                break;
            ...
            case 'ArrowRight': // Move right along the x-axis
                cube.position.x += 1;
                break;
            case 'w': // Move forward along the z-axis
                cube.position.z += 1;
                break;
            case 's': // Move backward along the z-axis
                cube.position.z -= 1;
                break;
        }
        updateCubeOutline(); // Update the outline position
        console.log(`Cube Position: x=${cube.position.x}, y=${cube.position.y}, z=${cube.position.z}`);
    });
...
// Function to update the cube outline position
function updateCubeOutline() {
    cubeOutline.position.copy(cube.position); // Sync outline position with cube position
}
